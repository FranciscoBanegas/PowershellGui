...
$btn.Add_Click({ 
    $selectedItems = $lv.CheckedItems
    if($SelectedItems) {
        foreach($item in $selectedItems) {
          if ($item.SubItems[0].Text -eq "Get Services") {
            [System.Windows.Forms.MessageBox]::Show('Soy el elemento seleccionado' + $item)
          }elseif ($item.SubItems[0].Text -eq "Bypass Windows 11 TPM") {
            [System.Windows.Forms.MessageBox]::Show('Soy el elemento seleccionado' + $item)
          }
        }
    }else {
